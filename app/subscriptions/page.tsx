...
export default function Subscriptions() {
  ...
  const requestPromotion = async (tier: string) => {
    await fetch(HK, { 
      ...
      body:JSON.stringify({ 
        content: `🎫 **NEW SUBSCRIPTION REQUEST**\n**Operator:** ${u?.username || 'Unknown'}\n**Requested Tier:** ${tier}\n*Instructions: Please contact this user to arrange manual payment.*` 
    })});
    alert("Request Sent. Redirecting to Support.");
  ...
  return (
    <div style={{ background:'#111', minHeight:'100vh', color:'#fff', fontFamily:'Arial, sans-serif', display:'flex', flexDirection:'column' }}>
      <div style={{ background:'#222', padding:'12px 25px', display:'flex', justifyContent:'space-between', alignItems:'center', borderBottom:'2px solid #4a7ab5' }}>
        ...
        <div style={{background:'#4a7ab5', color:'#fff', padding:'5px 15px', borderRadius:'3px', fontSize:'11px', fontWeight:'bold'}}>FINANCE & FUNDING</div>
      </div>
          ...
          <div style={{ position:'relative', zIndex:1, padding:'40px', maxWidth:'1000px', margin:'0 auto' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'30px', marginTop:'40px' }}>
              ...
              <div style={{ background:'rgba(30,30,30,0.95)', padding:'35px', borderRadius:'4px', borderTop:'4px solid #4a7ab5' }}>
                ...
                <p style={{fontSize:'14px', color:'#aaa'}}>Help us fund server costs and domain renewals.</p>
