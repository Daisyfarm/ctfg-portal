...
import { Wallet, Tractor, Send, Map, Clock, Briefcase, Landmark, LogOut, ChevronDown, Cloud, ShieldCheck, TrendingUp, FileCheck, UserCheck, Megaphone, Trophy, Radio, Star, LifeBuoy } from 'lucide-react';
...
export default function Dash() {
  ...
  const load = async () => {
    ...
    setP(pr); setTx(t || []); setNews(n?.[0]?.message || "Welcome!"); setRadio(rd?.[0] || { message: 'Radio Silence', sender: 'Dispatch' });
  ...
  if (ld || !p) return <div style={{background:'#111',color:'#fff',height:'100vh',display:'flex',alignItems:'center',justifyContent:'center'}}>Establishing Secure Link...</div>;
        ...
        <div style={{ flex:1, position:'relative', overflow:'hidden' }}>
          ...
          <div style={{ position:'relative', zIndex:1, padding:'40px', maxWidth:'1100px' }}>
                ...
                <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start'}}>
                    ...
                    <div style={{display:'flex', gap:'5px'}}>
                        {p.badges?.map((b:string) => (
                            <span key={b} style={{background:'#4a7ab5', padding:'4px 8px', borderRadius:'3px', fontSize:'9px', fontWeight:'bold', textTransform:'uppercase'}}>{b}</span>
                        ))}
                    </div>
                </div>
            ...
            <div style={{ background:'rgba(0,0,0,0.8)', padding:'20px', borderRadius:'4px', maxWidth:'720px' }}>
               ...
               {tx.length === 0 ? <p style={{color:'#444', fontSize:'12px'}}>No records found.</p> : tx.map(t => (
