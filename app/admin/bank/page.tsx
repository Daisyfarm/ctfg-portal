  ...
  return (
    <div style={{ background: '#0a0a0a', minHeight: '100vh', color: '#fff', padding: '40px' }}>
      ...
      <div style={{display:'flex', alignItems:'center', gap:'10px', borderBottom:'2px solid #dc2626', paddingBottom:'20px', marginBottom:'40px'}}>
        ...
        <h1>CTFG ADMIN BANK</h1>
      </div>
      {reqs.map(r => (
        <div key={r.id} style={{ background: '#111', padding: '20px', marginBottom: '15px', display:'flex', justifyContent:'space-between', alignItems:'center', border:'1px solid #222' }}>
          <div>
            <p style={{margin:0, color:'#555', fontSize:'10px'}}>OPERATOR</p>
            <p style={{fontSize:'18px', fontWeight:'bold'}}>{r.profiles?.username}</p>
          </div>
          <div style={{textAlign:'center'}}>
            <p style={{margin:0, color:'#555', fontSize:'10px'}}>AMOUNT</p>
            <p style={{fontSize:'22px', fontWeight:'bold', color:'#eab308'}}>${r.amount?.toLocaleString()}</p>
          </div>
          <div style={{display:'flex', gap:'10px'}}>
            <button onClick={()=>handleAction(r.id, r.user_id, r.amount, r.profiles.balance, r.profiles.username, 'completed')} style={{background:'#22c55e', padding:'10px 20px', border:'none', fontWeight:'bold', cursor:'pointer'}}>APPROVE</button>
            <button onClick={()=>handleAction(r.id, r.user_id, r.amount, r.profiles.balance, r.profiles.username, 'denied')} style={{background:'#dc2626', color:'#fff', padding:'10px 20px', border:'none', fontWeight:'bold', cursor:'pointer'}}>DENY</button>
          </div>
        </div>
      ))}
      {reqs.length === 0 && <p style={{textAlign:'center', color:'#333'}}>No pending requests.</p>}
    </div>
  );
}
