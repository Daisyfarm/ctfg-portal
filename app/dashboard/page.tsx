    ...
    <div style={{ background:'#0b0f1a',minHeight:'100vh',color:'white',fontFamily:'sans-serif',padding:'15px' }}>
      <div style={{ maxWidth:'500px',margin:'0 auto' }}>
        <div style={{ display:'flex',gap:'8px',marginBottom:'15px',justifyContent:'center',flexWrap:'wrap' }}>
          <button onClick={load} style={{background:'#1e293b',color:'white',border:'none',padding:'10px',borderRadius:'10px'}}><RefreshCcw size={16}/></button>
          <button onClick={()=>window.location.href='/contracts'} style={{background:'#6366f1',color:'white',border:'none',padding:'8px 12px',borderRadius:'10px',fontWeight:'bold',fontSize:'12px'}}>Jobs</button>
          <button onClick={()=>window.location.href='/bank'} style={{background:'#22c55e',color:'white',border:'none',padding:'8px 12px',borderRadius:'10px',fontWeight:'bold',fontSize:'12px'}}>Bank</button>
          <button onClick={()=>window.location.href='/land'} style={{background:'#f97316',color:'white',border:'none',padding:'8px 12px',borderRadius:'10px',fontWeight:'bold',fontSize:'12px'}}>Land</button>
          {p?.rank!=='Farmer' && <button onClick={()=>window.location.href='/community'} style={{background:'#475569',color:'white',border:'none',padding:'8px 12px',borderRadius:'10px',fontWeight:'bold',fontSize:'12px'}}>Hub</button>}
          {p?.rank==='Admin' && <button onClick={()=>window.location.href='/admin'} style={{background:'#dc2626',color:'white',border:'none',padding:'8px 12px',borderRadius:'10px',fontWeight:'bold',fontSize:'12px'}}>Staff</button>}
        ...
        <div style={{ background:'#131926',padding:'15px',borderRadius:'15px',border:'1px solid #1e293b' }}>
          ...
          {tx.map(t => (
            <div key={t.id} style={{display:'flex',justifyContent:'space-between',fontSize:'11px',padding:'5px 0',borderBottom:'1px solid #1e293b'}}>
