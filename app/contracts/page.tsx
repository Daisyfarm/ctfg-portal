...
import { Briefcase, ArrowLeft, Clock, CheckCircle, Plus, Home, Star, DollarSign, MapPin, Cloud, LogOut, Map, Tractor } from 'lucide-react';
...
export default function Contracting() {
  ...
  const ref = async () => {
    ...
    const { data } = await sb.from('contracts').select('*, profiles!contracts_assigned_to_fkey(username), employer:profiles!contracts_employer_id_fkey(username, companies(name, logo_url))').order('created_at', { ascending: false });
    setJobs(data || []); setLd(false);
  };
  useEffect(() => { ref(); }, []);
  ...
  if (ld || !u) return <div style={{background:'#111',color:'#fff',height:'100vh',display:'flex',alignItems:'center',justifyContent:'center'}}>Loading Logistics...</div>;
        ...
        <div style={{ flex:1, background:'rgba(20,20,20,0.8)', padding:'30px', overflowY:'auto' }}>
          ...
          {jobs.map(j => (
            <div key={j.id} style={{ background:'rgba(50,50,50,0.9)', padding:'20px', borderRadius:'4px', border:'1px solid #444', display:'flex', gap:'20px', alignItems:'center', marginBottom:'10px' }}>
              <div style={{ width:'80px', height:'80px', background:'#fff', borderRadius:'4px', display:'flex', alignItems:'center', justifyContent:'center', padding:'5px' }}>
              ...
              <div style={{ flex:1 }}>
                ...
                <p style={{ margin:'5px 0', fontSize:'10px', color:'#aaa' }}>BY: {j.employer?.companies?.name || j.employer?.username}</p>
