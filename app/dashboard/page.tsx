...
export default function Dash() {
  ...
  const load = async () => {
    ...
    const totalDebt = ln?.reduce((acc: number, curr: any) => acc + curr.amount_remaining, 0) || 0;
    setDebt(totalDebt);
  ...
  if (!p) return <div style={{background:'#0b0f1a',color:'#fff',height:'100vh',display:'flex',alignItems:'center',justifyContent:'center'}}>Syncing CTFG Portal...</div>;
    ...
    <div style={{ background:'#0b0f1a', minHeight:'100vh', color:'#fff', padding:'20px', fontFamily:'sans-serif', textAlign:'center' }}>
      <div style={{ maxWidth:'500px', margin:'0 auto' }}>
        ...
        <div style={{ display:'flex', justifyContent:'center', gap:'10px', margin:'15px 0' }}>
          <div style={{ background:'#131926', padding:'8px 12px', borderRadius:'12px', border:'1px solid #1e293b', textAlign:'left', minWidth:'120px' }}>
            ...
            {s?.slots?.players?.filter((pl:any)=>pl.isUsed).map((pl:any)=><p key={pl.name} style={{margin:'4px 0 0 0',fontSize:'10px',color:'#22c55e'}}>🚜 {pl.name}</p>)}
          </div>
          <div style={{ background:'#131926', padding:'10px 15px', borderRadius:'12px', border:'1px solid #1e293b', display:'flex', alignItems:'center', gap:'10px' }}>
            <Cloud size={14} color="#3b82f6" /> <span style={{fontSize:'12px', fontWeight:'bold'}}>{w || '--°F'} Montana</span>
          </div>
        </div>

        <div style={{ background:'rgba(34,197,94,0.1)', border:'1px solid #22c55e', padding:'10px', borderRadius:'12px', marginBottom:'15px', display:'flex', alignItems:'center', gap:'10px' }}>
        ...
        <div style={{ display:'flex', justifyContent:'center', gap:'6px', flexWrap:'wrap', marginBottom:'20px' }}>
          ...
          <button style={{...btn, background:'#444'}} onClick={()=>sb.auth.signOut().then(()=>window.location.href='/')}><LogOut size={12}/> Logout</button>
        </div>
