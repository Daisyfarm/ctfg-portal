...
export default function LiveMap() {
  ...
  if (!L) return <div style={{background:'#000',color:'#fff',height:'100vh',display:'flex',alignItems:'center',justifyContent:'center'}}>Syncing Satellite...</div>;

  // Judith Plains 4x uses a 4096 unit grid (-2048 to 2048)
  ...
  return (
    <div style={{ background:'#000', minHeight:'100vh' }}>
      ...
      <MapContainer crs={L.CRS.Simple} bounds={[[0,0],[100,100]]} style={{ height: '100vh', width: '100%' }} zoom={3} center={[50,50]}>
        ...
        {/* FIELD DOTS - Placed using exact server coordinates */}
        {data?.fields?.map((f:any) => {
          ...
          return (
            <CircleMarker 
              ...
              radius={owned ? 2 : 5} 
              pathOptions={{ color: owned ? '#3b82f6' : '#22c55e', fillColor: owned ? '#3b82f6' : '#22c55e', fillOpacity: 0.7 }}
            >
              <Popup>
                <div style={{color:'#000', textAlign:'center'}}>
                  <strong>Field {f.id}</strong><br/>
                  {owned ? "STATUS: OWNED" : `PRICE: $${info?.price?.toLocaleString() || '150,000'}`}
                </div>
              </Popup>
            </CircleMarker>
          );
        ...
        {data?.vehicles?.filter((v:any) => v.category !== "MISC").map((v:any, i:number) => (
          <Marker key={i} position={[100 - conv(v.z), conv(v.x)]} icon={L.divIcon({ html: `<div style="background:#22c55e; border:2px solid #fff; border-radius:50%; width:22px; height:22px; display:flex; align-items:center; justify-content:center; font-size:12px; box-shadow:0 0 10px #000;">🚜</div>`, className: '', iconSize: [22, 22] })}>
