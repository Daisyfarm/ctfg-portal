...
export default function Dash() {
  ...
  if (!m || !p) return <div style={{background:'#0b0f1a',color:'#fff',height:'100vh',display:'flex',alignItems:'center',justifyContent:'center'}}>Connecting...</div>;
  ...
  return (
    <div style={{ background:'#0b0f1a', minHeight:'100vh', color:'#fff', padding:'20px', fontFamily:'sans-serif', textAlign:'center' }}>
      ...
      {/* SERVER STATUS - Fixed with ?. to prevent 'used' error */}
      <div style={{ background:'#131926', padding:'10px', borderRadius:'15px', margin:'15px auto', display:'inline-flex', alignItems:'center', gap:'10px', border:'1px solid #1e293b' }}>
        <div style={{ width:'8px', height:'8px', borderRadius:'50%', background: s?.slots ? '#22c55e' : '#444' }}></div>
        <p style={{ margin:0, fontSize:'12px' }}>{s?.slots ? `${s.name}: ${s?.slots?.used}/${s?.slots?.capacity}` : 'Montana Server Offline'}</p>
      </div>

      <div style={{ background:'linear-gradient(135deg,#166534,#064e3b)', padding:'35px', borderRadius:'25px', margin:'0 auto 20px', maxWidth:'400px' }}>
        <h2 style={{ margin:0, fontSize:'45px' }}>${p?.balance?.toLocaleString() || '0'}</h2>
      </div>
      ...
      <div style={{ background:'#131926', padding:'15px', borderRadius:'15px', maxWidth:'400px', margin:'0 auto', textAlign:'left', border:'1px solid #1e293b' }}>
        ...
        {(tx || []).map(t => (
          <div key={t?.id} style={{ display:'flex', justifyContent:'space-between', fontSize:'11px', padding:'5px 0' }}>
