...
import { sb } from "../../../lib/supabase"; // Must be 3 levels!
...
export default function StaffAuthority() {
  ...
  if (ld) return (
    <div style={{background:'#111', color:'#d4af37', height:'100vh', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'serif'}}>
      ...
      <span style={{marginLeft:'10px'}}>AUTHORIZING ACCESS...</span>
    </div>
  );
    ...
    <div style={{ background: '#111', minHeight: '100vh', color: '#f5f5dc', padding: '40px', fontFamily:'serif' }}>
      ...
      <div style={{display:'flex', alignItems:'center', gap:'15px', borderBottom:'1px solid #d4af37', paddingBottom:'20px', marginBottom:'40px'}}>
        ...
        <div>
          ...
          <p style={{margin:0, fontSize:'10px', color:'#d4af37'}}>DAISY'S DREAM FARM NETWORK</p>
        </div>
      </div>

      {reqs.length === 0 ? (
        <p style={{textAlign:'center', color:'#333', marginTop:'50px'}}>No pending network logs.</p>
      ) : (
        reqs.map((r: any) => (
