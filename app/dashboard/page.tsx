...
export default function Dash() {
  ...
  const load = async () => {
    ...
    setP(prof); setTx(t || []);
    try { const r = await fetch('/api/server'); const d = await r.json(); setS(d.server); } catch (e) {}
    setLd(false);
  };
  useEffect(() => { load(); }, []);
    ...
    <div style={{ background:'#0b0f1a',minHeight:'100vh',color:'white',fontFamily:'sans-serif',padding:'20px' }}>
      <div style={{ maxWidth:'600px',margin:'0 auto' }}>
        ...
        <div style={{ background:'#131926',padding:'20px',borderRadius:'20px',marginBottom:'20px',display:'flex',alignItems:'center',gap:'15px',border:'1px solid #1e293b' }}>
          ...
          <p style={{margin:0,fontSize:'14px'}}>{s ? `${s.name}: ${s.slots.used}/${s.slots.capacity} Online` : 'Server Offline'}</p>
        </div>
