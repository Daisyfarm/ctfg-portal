    ...
    <div style={{ 
      ...
      <main style={{ maxWidth: '1280px', margin: '0 auto', padding: '40px 24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px' }}>
          {[
            { crop: 'Wheat (Grade A)', price: '$1,842 / T', change: '+4.2%', status: 'Bullish' },
            { crop: 'Yellow Corn', price: '$930 / T', change: '+1.5%', status: 'Stable' },
            { crop: 'Soybeans', price: '$2,150 / T', change: '-0.8%', status: 'Correction' },
            { crop: 'Barley', price: '$760 / T', change: '+2.9%', status: 'Active' },
          ].map((item, idx) => (
            <div key={idx} style={{ backgroundColor: 'rgba(11, 14, 20, 0.75)', backdropFilter: 'blur(10px)', border: '1px solid rgba(255, 255, 255, 0.15)', borderRadius: '12px', padding: '24px' }}>
              <div style={{ fontSize: '12px', fontWeight: 900, color: '#a1a1aa', textTransform: 'uppercase', marginBottom: '8px' }}>{item.crop}</div>
              <div style={{ fontSize: '32px', fontWeight: 900, color: '#ffffff', marginBottom: '8px' }}>{item.price}</div>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px', fontWeight: 900 }}>
                <span style={{ color: item.change.startsWith('+') ? '#34d399' : '#ef4444' }}>{item.change}</span>
