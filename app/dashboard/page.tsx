...
import { Wallet, Tractor, Send, Map, Clock, Briefcase, Landmark, LogOut, ChevronDown, Cloud, ShieldCheck, TrendingUp } from 'lucide-react';
...
export default function Dash() {
  ...
  const sideBtn = { width:'100%', padding:'12px 15px', background:'#4a7ab5', color:'#fff', border:'none', marginBottom:'8px', textAlign:'left' as const, cursor:'pointer', fontWeight:'bold', fontSize:'12px', borderRadius:'4px', display:'flex', alignItems:'center', gap:'10px' };
        ...
        <div style={{ flex:1, position:'relative', overflow:'hidden' }}>
          ...
          <div style={{ position:'relative', zIndex:1, padding:'40px' }}>
            <div style={{ display:'flex', gap:'20px' }}>
              ...
              {/* LEFT CARD: IDENTITY & BALANCE */}
              <div style={{ background:'rgba(0,0,0,0.8)', padding:'30px', borderRadius:'4px', width:'400px', borderLeft:'6px solid #4a7ab5' }}>
                <p style={{ margin:0, fontSize:'12px', color:'#888', textTransform:'uppercase', letterSpacing:'1px' }}>Welcome back, Operator</p>
              ...
              <div style={{ background:'rgba(0,0,0,0.8)', padding:'30px', borderRadius:'4px', width:'300px', borderLeft:'6px solid #22c55e' }}>
                ...
                <div style={{ display:'flex', alignItems:'center', gap:'10px', margin:'15px 0' }}>
                  <div style={{ width:'10px', height:'10px', borderRadius:'50%', background: s?.slots?.used > 0 ? '#22c55e' : '#ff4d4d', boxShadow: s?.slots?.used > 0 ? '0 0 10px #22c55e' : 'none' }}></div>
                ...
                {s?.slots?.players?.filter((pl:any)=>pl.isUsed).map((pl:any)=>(
                  <p key={pl.name} style={{fontSize:'11px', color:'#22c55e', margin:'5px 0'}}>🚜 {pl.name} ({pl.uptime}m)</p>
                ))}
              </div>
