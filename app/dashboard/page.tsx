...
import { Wallet, Tractor, RefreshCcw, Send, Map, Clock, CloudSun } from 'lucide-react';
...
export default function Dash() {
  ...
  const load = async () => {
    ...
    // 1. Fetch Montana Server Status
    fetch('/api/server').then(r=>r.json()).then(d=>{if(d?.server) setS(d.server)}).catch(()=>null);
    
    // 2. Fetch Real-World Montana Weather (Judith Basin)
    fetch('https://api.open-meteo.com/v1/forecast?latitude=47.15&longitude=-110.22&current=temperature_2m,weather_code').then(r=>r.json()).then(data=>setW(data.current)).catch(()=>null);
  };
  useEffect(() => { load(); }, []);
  ...
  return (
    <div style={{ background:'#0b0f1a', minHeight:'100vh', color:'#fff', padding:'15px', fontFamily:'sans-serif', textAlign:'center' }}>
      ...
      <div style={{ display:'flex', justifyContent:'center', gap:'10px', margin:'15px 0' }}>
        <div style={{ background:'#131926', padding:'8px 12px', borderRadius:'12px', border:'1px solid #1e293b', display:'flex', alignItems:'center', gap:'8px' }}>
          <div style={{ width:'8px', height:'8px', borderRadius:'50%', background: s?'#22c55e':'#ef4444' }}></div>
          <span style={{fontSize:'11px'}}>{s ? `${s.slots.used}/${s.slots.capacity} Online` : 'Offline'}</span>
        </div>
        <div style={{ background:'#131926', padding:'8px 12px', borderRadius:'12px', border:'1px solid #1e293b', display:'flex', alignItems:'center', gap:'8px' }}>
          <CloudSun size={14} color="#3b82f6" />
          <span style={{fontSize:'11px'}}>{w ? `${w.temperature_2m}°C Montana` : 'Loading Weather...'}</span>
        </div>
      </div>
