...
import { Truck, CheckCircle, Clock, MapPin, Radio, Tractor, Briefcase, Landmark, LogOut, Cloud } from 'lucide-react';
...
export default function DispatchCenter() {
  ...
  const verifyDelivery = async (job: any) => {
    ...
    if (error) alert(error.message);
      ...
      alert("Delivery Verified."); load();
    }
  };

  if (ld || !u) return <div style={{background:'#111',color:'#fff',height:'100vh',display:'flex',alignItems:'center',justifyContent:'center'}}>Accessing Dispatch Terminal...</div>;
    ...
    <div style={{ background:'#111', minHeight:'100vh', color:'#fff', fontFamily:'Arial', display:'flex', flexDirection:'column' }}>
      ...
      <div style={{ display:'flex', flex:1 }}>
        <div style={{ width:'240px', background:'#222', padding:'20px', borderRight:'1px solid #000' }}>
          ...
          <button style={{...sideBtn, background:'#333', color:'#fff'}} onClick={()=>window.location.href='/dispatch'}><Radio size={16} color="#3b82f6"/> Dispatch Center</button>
          <button style={sideBtn} onClick={()=>window.location.href='/admin'}>Staff Panel</button>
          <button style={{...sideBtn, marginTop:'20px'}} onClick={()=>sb.auth.signOut().then(()=>window.location.href='/')}>Logout</button>
        </div>
        ...
        <div style={{ flex:1, background:'#1a1a1a', padding:'40px' }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:'15px' }}>
                {activeHauls.length === 0 ? <p style={{color:'#555'}}>No active truck loads.</p> : activeHauls.map(job => (
                    <div key={job.id} style={{ background:'rgba(40,40,40,0.9)', padding:'25px', borderRadius:'4px', borderLeft:'5px solid #f59e0b', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                        <div>
                            ...
                            <p style={{margin:0, fontSize:'13px', color:'#aaa'}}><MapPin size={14}/> DESTINATION: {job.destination}</p>
                        </div>
                        <div style={{ textAlign:'right' }}>
                            <p style={{ margin:0, fontSize:'24px', color:'#22c55e', fontWeight:'bold' }}>${job.payout.toLocaleString()}</p>
                            <button onClick={()=>verifyDelivery(job)} style={{marginTop:'10px', padding:'10px 25px', background:'#22c55e', color:'#000', border:'none', fontWeight:'bold', cursor:'pointer', fontSize:'11px', borderRadius:'2px'}}>CONFIRM & PAY</button>
                        </div>
                    </div>
                ))}
            </div>
