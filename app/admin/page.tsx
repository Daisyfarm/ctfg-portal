...
export default function Admin() {
  ...
  return (
    <div style={{ background:'#000', minHeight:'100vh', color:'#fff', padding:'20px', fontFamily:'sans-serif' }}>
      ...
      <div style={{ margin:'20px 0', padding:'15px', border:'1px solid #22c55e', borderRadius:'10px' }}>
        <p style={{margin:0, color:'#22c55e', fontSize:'12px'}}>POST NEWS</p>
        <input placeholder="Announcement..." value={n} onChange={e=>setN(e.target.value)} style={{width:'70%', padding:'10px'}} />
        <button onClick={updateNews}>Post News</button>
      </div>

      <div style={{ margin:'20px 0', padding:'15px', border:'1px solid #333', borderRadius:'10px' }}>
        <p style={{margin:0, color:'#94a3b8', fontSize:'12px'}}>NEW CONTRACT</p>
        <input placeholder="Job Title" value={t} onChange={e=>setT(e.target.value)} />
        <input placeholder="$" type="number" value={p} onChange={e=>setP(e.target.value)} />
        <button onClick={postJob}>Post</button>
      </div>

      <h3>Pending</h3>
      {js.map(j => <div key={j.id} style={{ border:'1px solid #333', padding:'10px', marginBottom:'5px' }}>{j.profiles.username}: {j.title} <button onClick={()=>pay(j)}>Pay ${j.payout}</button></div>)}
      
      <h3>Registry</h3>
      {ps.map(f => <div key={f.id} style={{fontSize:'13px'}}>{f.username}: ${f.balance?.toLocaleString()} ({f.rank})</div>)}
    </div>
  );
}
