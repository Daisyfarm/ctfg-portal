    ...
    <div style={{ background: '#0a0a0a', minHeight: '100vh', color: '#fff', padding: '40px' }}>
      ...
      {reqs.map((r: any) => (
        <div key={r.id} style={{ background: '#111', padding: '20px', marginBottom: '15px', display:'flex', justifyContent:'space-between', alignItems:'center', border:'1px solid #222', borderRadius:'8px' }}>
          ...
          <div style={{textAlign:'center'}}><p style={{margin:0, color:'#555', fontSize:'10px'}}>AMOUNT</p><p style={{fontSize:'22px', fontWeight:'bold', color:'#eab308', margin:0}}>${r.amount?.toLocaleString()}</p></div>
          <div style={{display:'flex', gap:'10px'}}>
            <button onClick={()=>handleAction(r.id, r.user_id, r.amount, r.profiles?.balance || 0, r.profiles?.username || 'User', 'completed')} style={{background:'#22c55e', color:'#000', padding:'10px 20px', border:'none', fontWeight:'bold', cursor:'pointer', borderRadius:'4px'}}>APPROVE</button>
            <button onClick={()=>handleAction(r.id, r.user_id, r.amount, r.profiles?.balance || 0, r.profiles?.username || 'User', 'denied')} style={{background:'#dc2626', color:'#fff', padding:'10px 20px', border:'none', fontWeight:'bold', cursor:'pointer', borderRadius:'4px'}}>DENY</button>
          </div>
        </div>
      ))}
